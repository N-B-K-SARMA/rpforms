...
exports.default = {
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .addStringOption(option => option.setName('form_id').setDescription('The ID of the form').setRequired(true)))
        .addSubcommand(subcommand => subcommand.setName('status').setDescription('View RPForms system status'))
        .addSubcommand(subcommand => subcommand.setName('doctor').setDescription('Run system diagnostics'))
        .addSubcommand(subcommand => subcommand.setName('stats').setDescription('View application statistics')),
    async execute(interaction, client) {
        ...
        if (subcommand === 'reload') {
            ...
            if (RPForms_1.RPForms.config.getAll().embeds.banner)
                embed.setImage(RPForms_1.RPForms.config.getAll().embeds.banner);
            if (RPForms_1.RPForms.config.getAll().embeds.logo)
                embed.setThumbnail(RPForms_1.RPForms.config.getAll().embeds.logo);
            ...
            const stats = await RPForms_1.RPForms.database.getGlobalStats();
            const embed = new discord_js_1.EmbedBuilder()
                .setTitle('RPForms Status')
                .addFields({ name: 'Uptime', value: `${Math.floor(uptime / 3600)}h ${Math.floor((uptime % 3600) / 60)}m`, inline: true }, { name: 'RAM Usage', value: `${Math.round(memory.rss / 1024 / 1024)} MB`, inline: true }, { name: 'Loaded Forms', value: `${RPForms_1.RPForms.forms.getForms().length}`, inline: true }, { name: 'Pending Apps', value: `${stats.pending}`, inline: true }, { name: 'Database', value: `🟢 Connected`, inline: true }, { name: 'Version', value: `v1.0.0`, inline: true })
                .setColor(RPForms_1.RPForms.config.getAll().embeds.colors.primary)
                .setTimestamp();
            await interaction.reply({ embeds: [embed], ephemeral: true });
        }
            ...
            // Database
            try {
                await RPForms_1.RPForms.database.query('SELECT 1');
            ...
            for (const form of forms) {
                if (form.review.channelId && !guild.channels.cache.get(form.review.channelId))
                    missingChannels++;
                if (form.actions.onApprove.logChannelId && !guild.channels.cache.get(form.actions.onApprove.logChannelId))
                    missingChannels++;
                if (form.actions.onReject.logChannelId && !guild.channels.cache.get(form.actions.onReject.logChannelId))
                    missingChannels++;
            ...
            const embed = new discord_js_1.EmbedBuilder()
                ...
                .setColor(issues > 0 ? RPForms_1.RPForms.config.getAll().embeds.colors.warning : RPForms_1.RPForms.config.getAll().embeds.colors.success);
            ...
            const embed = new discord_js_1.EmbedBuilder()
                ...
                .addFields({ name: 'Total Applications', value: `${stats.total}`, inline: true }, { name: 'Approved', value: `${stats.approved}`, inline: true }, { name: 'Rejected', value: `${stats.rejected}`, inline: true }, { name: 'Pending Review', value: `${stats.pending}`, inline: true }, { name: 'Closed/Cancelled', value: `${stats.closed}`, inline: true })
                .setColor(RPForms_1.RPForms.config.getAll().embeds.colors.primary);
            await interaction.reply({ embeds: [embed], ephemeral: true });
        }
    },
