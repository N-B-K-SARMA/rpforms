    ...
    async execute(interaction, client) {
        if (interaction.isChatInputCommand()) {
            ...
            if (!handler) {
                ...
                handler = client.interactions.find((i) => interaction.customId.startsWith(i.id) && i.type === 'button_prefix');
            }
            if (handler) {
                try {
            ...
            if (!handler) {
                const prefix = interaction.customId.split('_')[0];
                handler = client.interactions.find((i) => interaction.customId.startsWith(i.id) && i.type === 'modal_prefix');
            }
            if (handler) {
                try {
                    await handler.execute(interaction, client);
